...
if [ $cluster_exists -eq 0 ]; then
  ...
  bash $SCRIPT_DIR/shell.sh "${candidate}" 'delete-environment' || true

  bash $SCRIPT_DIR/exec.sh "${candidate}" 'eksctl delete cluster --name ${EKS_CLUSTER_NAME} --region ${AWS_REGION} --wait --force --disable-nodegroup-eviction --timeout 45m'
else
